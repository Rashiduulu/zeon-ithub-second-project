...
const addToFsvorite = (btn) => {
    const localPokes = localStorage.getItem("favorites");
    if (localPokes) {
      localStorage.setItem("data", JSON.stringify([...localPokes, btn.dataset.pokemonId])
      );
    } else {
        localStorage.setItem("data", JSON.stringify([btn.dataset.pokemonId]))
    }
