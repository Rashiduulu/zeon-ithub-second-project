...
function getPokemonInfo(e) {
  ...
  if (e.target.classList.contains("recipe-btn")) {
    ...
    fetch(
      `https://pokeapi.co/api/v2/pokemonmealItem${pokeItem.dataset.id}`
    )
      .then((response) => response.json())
      .then((data) => infoPokemon(data.results));
  }
}
...
const infoPokemon = (pokemon) => {
  ...
  html += `<div class="card"> 
        <div class="left-side">
          ...
          <h4 class="poke-name">${pokemon.weight}</h4>
        </div>
        <div class="right-side">
        <button class="info-btn">Info</button>
        </div>
...
//       .then((data) => pokeInfoModal(data.results));
//   }
// }
